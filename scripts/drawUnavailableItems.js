export default function drawUnavailableItems (products) {
  const absentProductContainer = document.querySelector(".basket_absentProducts");
  let allAbsentProductsString = "";


  products.forEach((element) => {
    const productAbsentHTML = `<div class="basket_absentItem">
    <div class="basket_absentInfo">
    <div class="basket_absentImgWrap">
    <img
    src="${element.img}"
    alt="first-absent"
    ...
    ${ element.color || element.size ? `<div class="basket_absentCaptionColor">
    ${element.color ? `<p>Цвет: ${element.color} </p>` : ''}

        ${ element.size ? `<p class="basket_productCaptionSize">Размер: ${element.size}</p>` : ''}
      </div>` : ''}
    </div>
    </div>
    <div class="basket_absentBtns">
    <svg
